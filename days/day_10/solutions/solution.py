...
def parse_line(line: str) -> Tuple[List[bool], List[List[int]], List[int]]:
    ...
    target_pattern = [c == '#' for c in pattern_str]
...
def integer_gaussian_elimination(matrix: List[List[int]], target: List[int]) -> Tuple[List[List[int]], List[int], List[int], List[int]]:
    ...
    pivot_cols = []  # Columns that have pivots
    free_cols = []   # Free variables (columns without pivots)
    pivot_row = 0
        ...
        for row in range(num_rows):
            ...
            if A[row][col] != 0:
                ...
                if A[row][col] % pivot_val == 0:
                    ...
                    A[row] = [A[row][c] - factor * A[pivot_row][c] for c in range(num_cols)]
                    ...
                    if g > 0:
                        ...
                        factor1 = A[row][col] // g
                        factor2 = pivot_val // g
                        new_row = [factor1 * A[pivot_row][c] - factor2 * A[row][c] for c in range(num_cols)]
...
def solve_part2_ilp(buttons: List[List[int]], joltages: List[int], target_pattern: List[bool]) -> Optional[int]:
    ...
    for p in presses:
        ...
        solver.add(p <= total_joltage)
    
    # Add constraints: for each light, sum of presses of buttons affecting it = joltage
    for light_idx in range(num_lights):
        joltage_sum = z3.Sum([presses[btn_idx] for btn_idx in range(num_buttons) 
                              if light_idx in buttons[btn_idx]])
        solver.add(joltage_sum == joltages[light_idx])
    ...
    if solver.check() == z3.sat:
        ...
        for btn_idx, btn in enumerate(buttons):
            presses_val = model[presses[btn_idx]].as_long()
            for light in btn:
                if 0 <= light < num_lights:
...
def solve_part2_ilp_fallback(buttons: List[List[int]], joltages: List[int], target_pattern: List[bool]) -> Optional[int]:
    ...
    if all(j % 2 == 0 for j in joltages) and any(j > 0 for j in joltages):
        ...
        result = solve_part2_ilp_fallback(buttons, halved_joltages, target_pattern)
    ...
    reduced_A, reduced_b, pivot_cols, free_cols = integer_gaussian_elimination(matrix, joltages)
    ...
    if not free_cols:
        ...
        for i in range(len(pivot_cols) - 1, -1, -1):
            if i >= len(reduced_A):
                continue
            col = pivot_cols[i]
            pivot_val = reduced_A[i][col] if i < len(reduced_A) and col < len(reduced_A[i]) else 0
            if pivot_val == 0:
                continue
